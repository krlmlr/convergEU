...
ttmp <- cbind(outSig$res, dplyr::select(wkDF[estrattore,], -contains(timeName)))


# minimum and maximum values
rawDat <- dplyr::select(wkDF[estrattore,], -contains(timeName))
ttmp$serieMin <- apply(rawDat,1,min)
ttmp$serieMax <- apply(rawDat,1,max)  
# build labels for minimum and maximum
ttmp$eticheMin <-sapply(seq(1,length(ttmp$serieMin)),
          function(vx){
      estrattore <- ttmp$serieMin[vx] == rawDat[vx,]
      paste(names(rawDat)[estrattore],collapse="/")
})
ttmp$eticheMax <- sapply(seq(1,length(ttmp$serieMin)),
          function(vx){
...
myG2 <-  ggplot(ttmp) +  
         geom_line(aes(x=ttmp[,timeName], y =ttmp[,"mean"],colour="black" )) +
          ...
          # add countries
          geom_line( aes( x = ttmp[,timeName], y = ttmp[[seleCountry]], 
                          colour="red"),linetype="dotted") + 
          geom_point( aes(x = ttmp[,timeName], y = ttmp[,seleCountry], colour="red")) +
          ggplot2::scale_x_continuous(breaks = ttmp[,timeName],
                                      labels = ttmp[,timeName]) +
          ylim(c(miniY,maxiY)) + 
          xlab("Year") + 
          ylab("Indicator level") +
        ggplot2::geom_line(aes(x=ttmp[,timeName],
                               y=ttmp$serieMin,
                               colour = "blue" 
                               ),linetype="dashed") +
        ggplot2::geom_line(aes(x=ttmp[,timeName],
...
final_TB <- tidyr::gather(scobelvl, key="Country",value="Level",curCountries)
```
